...
export default function TrendIntelligenceAgent() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F8F6] text-[#0A0A0A] p-4 md:p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <header className="mb-12 border-b border-slate-200 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-xs font-semibold bg-gray-900 text-white px-3 py-1.5 rounded-full">
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 hover:border-slate-300 rounded-lg text-xs font-semibold text-slate-700 transition-all"
                  title="History"
                >
                ...
                <div className="flex items-center gap-2 rounded-full bg-gray-900 text-white px-3 py-1.5">
                  <span className="text-xs font-semibold">{user.displayName?.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase()}</span>
                  <button onClick={logout} className="ml-1 opacity-70 hover:opacity-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={login}
                  className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2 text-sm font-semibold rounded-lg hover:bg-gray-800 transition-all"
                >
          ...
          {showHistory && (
            <>
              ...
              <motion.div
                ...
                className="fixed right-0 top-0 bottom-0 w-full md:w-80 bg-white border-l border-slate-200 z-50 flex flex-col shadow-xl"
              >
                <div className="p-6 border-b border-slate-200 flex items-center justify-between bg-white">
                  <h3 className="text-sm font-semibold text-gray-900">Intelligence History</h3>
                  <button 
                    onClick={() => setShowHistory(false)} 
                ...
                <div className="flex-grow overflow-y-auto p-4 space-y-3">
                  {!user ? (
                    <div className="bg-white border border-slate-200 p-6 text-center space-y-4 rounded-xl">
                      <div className="w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center mx-auto">
                        <LogIn className="w-6 h-6 text-indigo-600" />
                      </div>
                      <div className="space-y-2">
                        <p className="font-semibold text-gray-900">Sign in to save history</p>
                        <p className="text-xs text-slate-500">Keep track of your market signals and execution plans across sessions.</p>
                      </div>
                      <button
                        onClick={login}
                        className="w-full flex items-center justify-center gap-2 bg-gray-900 text-white px-4 py-2.5 text-sm font-semibold rounded-lg hover:bg-gray-800 transition-all"
                      >
                    ...
                    history.map((item) => (
                      <div 
                        key={item.id} 
                        className="group relative bg-white border border-slate-200 p-4 hover:border-slate-300 hover:shadow-sm rounded-lg transition-all cursor-pointer"
                        onClick={() => {
                          setResult(item);
                          if (item.marketMode) {
                  ...
                  <div className="p-4 border-t border-slate-200 bg-white">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-grow">
                        <p className="text-xs font-semibold text-gray-900 leading-none">{user.displayName}</p>
        ...
        <DeepDiveModal 
          ...
          setActiveDeepDiveTab={setActiveDeepDiveTab as (tab: 'plan' | 'costs' | 'grants' | 'checklist' | 'investors' | 'strategy') => void}
          generateDeepDive={generateDeepDive}
        ...
        {/* Footer Info */}
        {!result && !loading && (
          <footer className="mt-24 border-t border-slate-200 pt-8 grid grid-cols-1 md:grid-cols-3 gap-8 opacity-50">
            <div>
              <p className="text-[10px] font-semibold uppercase text-slate-400 mb-2">System Status</p>
              <p className="text-xs">All intelligence modules operational. Monitoring global signals 24/7.</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase text-slate-400 mb-2">Methodology</p>
              <p className="text-xs">First-principles thinking applied to market inefficiencies and regulatory shifts.</p>
            </div>
            <div>
              <p className="text-[10px] font-semibold uppercase text-slate-400 mb-2">Disclaimer</p>
