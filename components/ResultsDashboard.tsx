...
export const ResultsDashboard: React.FC<ResultsDashboardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Analysis Summary */}
      <section id="step-2" className="scroll-mt-24 rounded-3xl border border-border/10 bg-gradient-to-b from-gray-50/80 to-white p-4 md:p-6">
        <div className="flex items-center justify-between gap-3 mb-4 pb-3 border-b border-border/10">
          <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-primary font-bold">Signal Desk · Lead Story</span>
          <span className="text-[10px] font-mono uppercase tracking-widest text-muted bg-white px-2 py-1 rounded-md border border-border/10">Edition: Live Analysis</span>
        </div>

        {/* Trend headline */}
        <div className="bg-white border border-border/20 p-4 md:p-8 rounded-3xl shadow-sm mb-4 md:mb-6">
          <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            <div className="lg:col-span-8 border-l-4 border-primary/60 pl-4 md:pl-6">
              <div className="flex items-center gap-2 text-primary mb-4">
                <Sparkles className="w-4 h-4" />
                <span className="text-xs font-mono uppercase font-bold tracking-wide">Emerging Trend Identified</span>
              </div>
              <h3 className="text-lg md:text-2xl font-sans font-semibold leading-snug mb-4 text-foreground w-full break-words [overflow-wrap:anywhere]">
                {result.trend}
              </h3>
              <p className="text-base text-muted leading-relaxed w-full break-words [overflow-wrap:anywhere]">{result.summary}</p>
            </div>

            <div className="lg:col-span-4 border-t lg:border-t-0 lg:border-l border-border/10 pt-4 lg:pt-0 lg:pl-6 bg-gray-50/70 rounded-2xl p-4">
              <p className="text-[10px] font-mono uppercase tracking-widest text-muted mb-3">Front Page Brief</p>
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-primary font-bold">Briefing Columns</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 items-stretch">
          <div className="bg-white border border-primary/15 p-4 md:p-8 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 h-full">
            ...
            <div className="grid grid-cols-1 gap-1.5">
              {result.affected_groups.map((g, i) => (
                <span key={i} className="w-full px-2 py-1 bg-primary/5 border border-primary/10 text-primary text-[10px] font-mono uppercase font-bold rounded-lg break-words [overflow-wrap:anywhere]">
                  {g}
                </span>
              ))}
            </div>
          ...
          <div className="bg-white border border-accent/20 p-4 md:p-8 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 h-full">
            ...
            <ul className="space-y-3">
              {result.problems.map((p, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-muted font-medium break-words [overflow-wrap:anywhere]">
